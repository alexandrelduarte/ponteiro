  ...
  return (
    <div className="tela-urna mt-4 rounded-cartao border border-tela-borda bg-tela p-4">
      ...
      <div className="mt-3 grid gap-3 text-sm md:grid-cols-3">
        <div className="rounded-controle bg-tela-fundo p-3">
          <div className="flex items-start justify-between gap-2">
            ...
            <span className="rounded-controle border border-tela-borda px-1.5 py-0.5 font-mono text-xs font-semibold whitespace-nowrap text-fosforo-forte">
          ...
          <p className="mt-1 text-xs leading-compacto text-fosforo">
            ...
            com chegada apertada ({fmtSinal(replay.r1L - replay.r1F)} p.p.) em vez dos{" "}
            {margem1T === null ? "–" : fmtSinal(margem1T)} das pesquisas. Probabilidade de Lula
            ainda chegar em 1º lugar: {Math.round(replay.pLider1 * 100)}%.
          </p>
        </div>
        ...
        <div className="rounded-controle bg-tela-fundo p-3">
          ...
          <p className="mt-1 text-xs leading-compacto text-fosforo">
            dos válidos (±1,6).{" "}
            <b>Vitória apertada de Lula por {fmtSinal(replay.r2L - replay.r2F)} p.p.</b> — réplica
        ...
        <div className="rounded-controle border border-confirma bg-tela-fundo p-3">
          ...
          <p className="mt-1 text-xs leading-compacto text-fosforo">
            ...
            hoje sob a réplica, o placar ~{fmt(replay.r2L, 0)}×{fmt(replay.r2F, 0)} seria quase
            certo (Lula ≈{Math.round(replay.elRepH * 100)}%).
          </p>
          <button
            type="button"
      ...
      <p className="mt-3 text-xs leading-compacto text-fosforo opacity-85">
        ...
        esse mesmo cenário aparece como ≈{Math.round(replay.pPainel * 100)}%, porque lá a incerteza
        sobre o próprio viés é mantida. A inversão da corrida exige uma hipótese que NÃO aconteceu
        em 2022: o erro do 1º turno persistir intacto na decisão — é o cartão «teste-limite +6,3»
